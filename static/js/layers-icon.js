...
export function drawIconCoreNews(ctx, size, t) {
    ...
    // 1. Outer pulse
    var outerRadius = r * (ICON_CONF.PULSE_START_RATIO + ICON_CONF.PULSE_SPREAD_RATIO * t);
    ctx.beginPath(); ctx.arc(x, y, outerRadius, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(0, 150, 255, " + (0.4 * (1 - t)) + ")";
    ctx.fill();

    // 2. Inner circular badge
    ctx.beginPath(); ctx.arc(x, y, r * ICON_CONF.INNER_RADIUS_RATIO, 0, Math.PI * 2);
    ctx.fillStyle = "#0072ff";
    ctx.shadowBlur = size * ICON_CONF.SHADOW_BLUR_RATIO; ctx.shadowColor = "rgba(0,0,0,0.4)";
    ctx.fill();
    ctx.shadowBlur = 0;

    // 3. "NEWS" Text
    var fontSize = Math.floor(size * 0.3);
    ctx.font = "bold " + fontSize + "px sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.strokeStyle = "black"; ctx.lineWidth = size * 0.1;
    ctx.strokeText("NEWS", x, y);
    ctx.fillStyle = "white"; ctx.fillText("NEWS", x, y);
...
export function drawIconCoreFire(ctx, size, t) {
    ...
    // 3. Flame Shape
    var s = r;
    var flicker = Math.sin(performance.now() / 60) * 1.5;
    ctx.beginPath();
    ...
    var pulseGrad = ctx.createLinearGradient(x, y - s, x, y + s);
    pulseGrad.addColorStop(0, "#ff4500"); pulseGrad.addColorStop(1, "#ffcc00");
...
export function drawIconCoreAlert(ctx, size, t) {
    ...
    // 2. Inner circular badge
    ctx.beginPath(); ctx.arc(x, y, r * ICON_CONF.INNER_RADIUS_RATIO, 0, Math.PI * 2);
    var grad = ctx.createRadialGradient(x, y, 0, x, y, r * ICON_CONF.INNER_RADIUS_RATIO);
    grad.addColorStop(0, "#ff4d4d"); grad.addColorStop(1, "#b30000");
    ctx.fillStyle = grad;
    ctx.shadowBlur = size * ICON_CONF.SHADOW_BLUR_RATIO; ctx.shadowColor = "rgba(0,0,0,0.4)";
    ctx.fill();
    ctx.shadowBlur = 0;
...
export function initUIPulsingIcons() {
    var canvases = document.querySelectorAll(".ui-pulsing-icon");
    if (canvases.length === 0) return;

    function animate() {
        var items = document.querySelectorAll(".ui-pulsing-icon");
        items.forEach(function (canv) {
            var type = canv.getAttribute("data-type");
            var size = canv.width;
            var ctx = canv.getContext("2d");
            var duration = type === "fire" ? 1500 : (type === "news" ? 2000 : 1200);
            var t = (performance.now() % duration) / duration;
